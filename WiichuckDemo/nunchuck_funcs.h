...
static void nunchuck_transform_data()
{ 
    joy_x_axis = nunchuck_buf[0];
    joy_y_axis = nunchuck_buf[1];
    accel_x_axis = nunchuck_buf[2]<<2; // * 2 * 2; 
    accel_y_axis = nunchuck_buf[3]<<2; // * 2 * 2;
    accel_z_axis = nunchuck_buf[4]<<2; // * 2 * 2;
...
// on the last 2 bits.  That is why I
// multiply them by 2 * 2
static void nunchuck_print_csvData()
{ 
    static int i=0;
  nunchuck_transform_data();
    Serial.print(i,DEC);
